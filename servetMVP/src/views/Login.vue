<template>
    <div class="flex flex-col items-center justify-center min-h-screen h-full bg-gray-50">
        <form @submit.prevent="handleSubmit" name="form" class="flex flex-col items-center space-y-2
            pt-16 pb-32 md:px-16 md:border md:rounded-md md:shadow-sm">
            <span class="pb-16 text-lg">Welcome to <b>Servet</b></span>
            <div class="w-full flex flex-col justify-center md:flex-row items-center md:space-x-4">
                <span class="md:min-w-sm w-full text-md capitalize">User name:<b>*</b></span>
            ...
            <div class="w-full flex flex-col justify-center md:flex-row items-center md:space-x-4">
                <span class="md:min-w-sm w-full text-md capitalize">Password:<b>*</b></span>
                <input class="md:min-w-[468px] w-full border border-slate-400 rounded-sm bg-gray-100 focus:ring-0
                    focus:border-slate-600 valid:bg-blue-50" type="password" required v-model="password" :class="errorMsg && 'highlight'">
            </div>
            <div class="w-full md:w-fit md:self-end flex flex-col items-center">
                <button class="w-full md:min-w-[468px] rounded-sm bg-[#F3ECD1] py-2 mt-8 transition-all
...
export default {
    ...
    methods: {
        ...
        getSession() {
            fetch("http://whosadevnow.tech/api/v1/getsession", {
                            credentials: "include",
                            })
                            .then((res) => res.json())
                            .then((data) => {
        ...
        csrf() {
            fetch("http://whosadevnow.tech/api/v1/getcsrf", {
                    credentials: "include",
                    })
                    .then((res) => {
        ...
        login(data) {
          console.log(data)
            fetch("http://whosadevnow.tech/api/v1/login", {
                method: "POST",
                headers: {
        ...
        logout() {
            fetch("http://whosadevnow.tech/api/logout", {
                credentials: "include",
                })
                .then(() => {
                    this.isAuthenticated = false;
