...
export default {
    ...
    methods: {
        ...
        getSession() {
            fetch("http://whosadevnow.tech/api/getsession", {
                            credentials: "include",
                            })
                            .then((res) => res.json())
                            .then((data) => {
        ...
        csrf() {
        // async getCSRF() {
            // let res = await fetch('http://whosadevnow.tech/api/v1/getcsrf')
            // // const data = await res.json()
            // console.log('csrf: ', res.headers.get('x-csrftoken'))
            // this.csrf = res.headers.get('x-csrftoken')
            // // let ses = await fetch('http://whosadevnow.tech/api/v1/getsession')
            // // let sesdata = await ses.json()
            // // console.log('session: ', sesdata)

            fetch("http://whosadevnow.tech/api/getcsrf", {
                    credentials: "include",
                    })
                    .then((res) => {
        ...
        //     let res = await fetch('http://whosadevnow.tech/api/v1/customers/login', {
        //         method: 'POST',
        ...
        // }
        login() {
            fetch("http://whosadevnow.tech/api/login", {
                method: "POST",
                headers: {
        ...
        whoami() {
            fetch("http://whosadevnow.tech/api/data", {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
